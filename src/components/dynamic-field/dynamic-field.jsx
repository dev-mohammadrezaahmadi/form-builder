...
export function SimpleField({ node, path }) {
  ...
  if (restriction?.restriction_type === 'enum_restriction') {
    return (
      <Field.Select name={path} label={labelText} InputProps={{ endAdornment: tooltipAdornment }}>
  ...
  if (restriction?.base === 'dateTime') {
    return <Field.DatePicker name={path} label={labelText} />;
  }
  ...
  return (
    <Field.Text name={path} label={labelText} InputProps={{ endAdornment: tooltipAdornment }} />
  );
}
  ...
  return (
    ...
    >
      <Stack direction="row" alignItems="center" spacing={1} padding={1.5}>
        ...
        <Typography fontWeight="bold">{verbose_name || name}</Typography>
...
export function ChoiceField({ node, path }) {
  ...
  return (
    <Box sx={{ mb: 2 }}>
      ...
      {isEnabled && (
        <Card
          sx={{ p: 2, bgcolor: '#eeeeeeff', border: '1px solid', borderColor: 'divider' }}
          elevation={0}
        >
          <Select
            fullWidth
          ...
          >
            {options.map((opt, idx) => (
              <MenuItem key={opt.name} value={idx}>
